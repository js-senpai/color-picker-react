...
const ColorList = ({toggleColor = (type:string,color: string) => console.log(color),colors = [{id: 1,color: '#000',name: 'black'}]}) => {
  return (
              ...
              <ul className="color-list__nav">
                  {
                      colors.map(({id,color,name}) =>(
                          <li key={id} onClick={() => toggleColor('hex',color)} className="color-list__nav-item">
